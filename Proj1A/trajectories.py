...
class LinearTrajectory(Trajectory):
    ...
    def __init__(self):
        ...
        """
        Trajectory.__init__(self, np.pi)
        self.position = [0, 0, 0]
    ...
    def target_pose(self, time):
        ...
        7x' :obj:`numpy.ndarray`
            desired configuration in workspace coordinates of the end effector
        """
        #return [time, 0, 0, 0, 1, 0, 0]
        self.position[0] += self.target_velocity(time)[0]*time
        return [self.position[0], 0, 0, 0, 1, 0, 0]
...
class CircularTrajectory(Trajectory):
    ...
    def __init__(self, center_position, radius, total_time):
        ...
        self.alpha = 0
        self.omega = 2*np.pi/self.total_time
        self.theta = 0
    ...
    def target_pose(self, time):
        ...
        self.position += [self.radius*np.cos(self.theta), self.radius*np.sin(self.theta), 0]
        return [self.position[0], self.position[1], 0, 0, 1, 0, 0]
    ...
    def target_velocity(self, time):
        ...
        return [-self.radius*np.sin(self.theta)*2*np.pi, self.radius*np.cos(self.theta)*2*np.pi, 0, 0, 0, 1]
        #w = np.array([0, 0, self.omega])
        #r = np.array(self.center) - np.array(self.target_pose[0:3])
        #x_d_dot = np.dot()
...
def define_trajectories(args):
    ...
    if args.task == 'line':
        ...
        trajectory = CircularTrajectory([0, 0, 0], 2, np.pi)
